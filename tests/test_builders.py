...
def test_symmetric_height0():
    topo = symmetric_topology(height=0, degree=2)
    assert topo.size == 1
    assert topo.depth == 0
...
def test_symmetric_ternary_height2_has_13_nodes():
    topo = symmetric_topology(height=2, degree=3)
    # 1 + 3 + 9 = 13
    assert topo.size == 13
    assert topo.depth == 2
    assert topo.equal_degree is True
...
def test_symmetric_degree1_is_a_chain():
    topo = symmetric_topology(height=3, degree=1)
    assert topo.size == 4
    assert topo.depth == 3
...
def test_symmetric_rejects_negative_height():
    with pytest.raises(ValueError):
        symmetric_topology(height=-1, degree=2)


def test_symmetric_rejects_zero_degree():
    with pytest.raises(ValueError):
        symmetric_topology(height=2, degree=0)
