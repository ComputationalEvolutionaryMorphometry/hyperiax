...
def add_parent(
    parent_formatter: TreeNodeFormatter, children_formatter: TreeNodeFormatter
) -> TreeNodeFormatter:
    """
    ...
    """
    parent_middle, children_middle = (
        parent_formatter.middle_width,
        children_formatter.middle_width,
    )
    ...
    if parent_middle == children_middle:  # Only one child
        ...
        lines = [
            padding + line for line in parent_formatter.lines
        ] + children_formatter.lines
        parent_width += children_middle - parent_middle
        middle = children_middle
    else:  # Have no child
        padding = " " * (parent_middle - children_middle)
        lines = parent_formatter.lines + [
            padding + line for line in children_formatter.lines
        ]
...
class TreeFormatter:
    ...
    def add_name(
        self,
        name: str,
        node_formatter: TreeNodeFormatter,
        parent_adder: callable = add_parent,
        seperator: str = " ",
    ) -> TreeNodeFormatter:
        if name:
            name_formatter = TreeNodeFormatter.from_string(str(name))
            node_formatter = parent_adder(
                TreeNodeFormatter.from_string(seperator), node_formatter
            )
    ...
    def tree_join_formatter(
        self, node: TopologyNode, depth: int = 0
    ) -> TreeNodeFormatter:
        """
        Recursively formats the tree structure and returns the root node formatter.
        ...
        if children:
            children_formatters = [
                self.tree_join_formatter(child, depth=depth + 1) for child in children
            ]
            if len(children) == 1:
                children_node_formatter = children_formatters[0]
                children_node_formatter.lines.insert(
                    0, " " * children_node_formatter.middle_width + "│"
                )
            else:
